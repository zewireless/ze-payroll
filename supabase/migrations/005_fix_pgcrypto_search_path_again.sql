-- ============================================
-- ze-payroll: re-fix pgcrypto search_path on record_attendance_punch
--
-- 002_fix_pgcrypto_schema.sql added `extensions` to this function's
-- search_path so crypt()/gen_salt() (installed by Supabase into the
-- `extensions` schema) would resolve. 003_geofence_attendance.sql and
-- 004_fix_geofence_round_error.sql both redefined this same function
-- (to add geofencing, then to fix a round() bug) using
-- `set search_path = public` only - silently dropping the 002 fix.
--
-- Result: crypt() is invisible again inside record_attendance_punch,
-- causing "function crypt(text, text) does not exist" on every PIN
-- check at the kiosk (scan.html). set_employee_pin() is unaffected -
-- it was never touched by 003/004 and still has the correct path.
--
-- This migration reapplies `public, extensions` on top of the current
-- (geofence + round-fix) body of the function. No schema/data changes.
--
-- Run this in the Supabase SQL Editor as the `postgres` role, after
-- 004_fix_geofence_round_error.sql. Idempotent: safe to re-run.
-- ============================================
